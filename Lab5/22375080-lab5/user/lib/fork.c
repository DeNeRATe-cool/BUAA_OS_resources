...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if(!(perm & PTE_COW)) {
		user_panic("at function cow_entry(struct Trapframe *tf) in fork.c: perm doesn't have PTE_COW'");
	}
...
static void duppage(u_int envid, u_int vpn) {
	...
	if(r) syscall_mem_map(0, (void *)addr, 0, (void *)addr, perm);
...
int fork(void) {
	...
	for (i = 0; i < USTACKTOP; i += PDMAP) {
		if(!(vpd[i >> PDSHIFT] & PTE_V)) continue;
		u_int j = i;
		while(j < i + PDMAP) {
			if(j >= USTACKTOP) break;
			if(vpt[VPN(j)] & PTE_V) duppage(child, VPN(j));
			j += PAGE_SIZE;
		}
	}	
