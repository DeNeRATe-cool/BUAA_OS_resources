...
// 加密状态
static struct EncryptState encrypt_state = {0, {0}};
...
// 加密一个数据块
void encrypt_block(void *block, const unsigned char *key) {
...
// 解密一个数据块
void decrypt_block(void *block, const unsigned char *key) {
	// 异或加密的加密和解密是相同的操作
	encrypt_block(block, key);
}

// 加载密钥
int load_key(const void *key_block) {
	// 检查密钥块是否有效
	if (*(uint32_t *)key_block != FS_MAGIC) {
		return -E_BAD_KEY;
	}
	
	// 复制密钥
	// 注意：fd2data 获取到的是文件的 FCB（File Control Block），即第一个数据块存放的是 FCB 信息，
	// 密钥内容实际存储在文件的第二个数据块中，需要先读取第二个数据块的数据。
	// 这里假设 key_block 指向的是整个文件映射的起始地址（即 FCB），
	// 所以密钥内容在 key_block + BLOCK_SIZE 处。
	memcpy(encrypt_state.key, (const unsigned char *)key_block + BLOCK_SIZE, KEY_SIZE);
	encrypt_state.key_loaded = 1;
	return 0;
}

// 卸载密钥
void unload_key(void) {
	encrypt_state.key_loaded = 0;
	memset(encrypt_state.key, 0, KEY_SIZE);
}

// 检查是否已加载密钥
int is_key_loaded(void) {
	return encrypt_state.key_loaded;
}
...
void write_block(u_int blockno) {
	...
	// 如果文件是加密的且已加载密钥,则加密数据
	if (encrypt_state.key_loaded) {
		encrypt_block(va, encrypt_state.key);
	}
...
int file_get_block(struct File *f, u_int filebno, void **blk) {
	...
	// 如果文件是加密的且已加载密钥,则解密数据
	if ((f->f_type & O_ENCRYPT) && encrypt_state.key_loaded) {
		decrypt_block(*blk, encrypt_state.key);
	}
